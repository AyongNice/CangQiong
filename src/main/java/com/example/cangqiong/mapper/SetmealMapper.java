...
public interface SetmealMapper {
    ...
    public List<SetmealDto> page(String name, String status, String categoryId, String storeId);

    @Select("select id, name,price,category_id as categoryId ,description,image from setmeal where id=#{id} and store_id = #{storeId}")
    ...
    @Update("update setmeal set name = #{name}, " +
            ...
            " price = #{price}, " +
            "status = #{status}," +
            " update_time=#{updateTime} where id = #{id} and store_id = #{updateUser}")
    ...
    //删除套餐
    public Integer deleteSetmeal(String[] ids, String storeId);


    @Update("update setmeal set status = #{status} where id = #{id} and store_id = #{storeId} ")
